...
class Streams():
    ACCEPTED_KEYS = ['time', 'distance', 'altitude', 'velocity_smooth', 'heartrate', 'cadence', 'watts', 'temp',
                     'moving', 'grade_smooth', 'lat', 'lng']
...
def run_server_and_wait_for_token_(client_id, client_secret, host='0.0.0.0', port=8001, session=None, event=None):
    """Runs a simple HTTP server to listen for the authorization callback and exchange the token."""
    import pdb
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        if url.path == "/authorization_successful":
            ...
            if access_token := data.get("access_token"):
                logger.debug(f"Authorized athlete: {access_token}")
    print('redirecting')
    return access_token



def run_server_and_wait_for_token(client_id, client_secret, host=None, port=None, address=None):
    """Runs a simple HTTP server to listen for the authorization callback and exchange the token."""
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        if host is not None and port is not None:
            s.bind((host, port))
        ...
        if url.path == "/authorization_successful":
            ...
            r = requests.post("https://www.strava.com/oauth/token", params)
            data = r.json()
            logger.debug(f"Authorized athlete: {data.get('access_token', 'Error retrieving token')}")
