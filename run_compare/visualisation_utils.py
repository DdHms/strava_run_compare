...
pio.renderers.default = "browser+jpg"
...
def mk_speed_and_hr_fig(distance, speed, hr):
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(x=distance, y=speed.tolist(), mode='lines', name='Speed',
                   line=dict(color='rgba(0,0,255,1)', width=2.5)))
    fig.update_layout(xaxis=dict(gridcolor='grey', dtick=500), yaxis=dict(gridcolor='grey', dtick=0.2))
    fig.update_layout(xaxis_title='Distance[m]', yaxis_title='Speed [min/km]', yaxis=dict(
        autorange='reversed'
    ))
    fig.write_image('speed_fig.jpg', format='jpeg', scale=3)
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(x=distance, y=hr.tolist(), mode='lines', name='HR',
                   line=dict(color='rgba(255,0,0,1)', width=2.5)))
    fig.update_layout(xaxis=dict(gridcolor='grey'), yaxis=dict(gridcolor='grey'))
    fig.update_layout(xaxis_title='Distance[m]', yaxis_title='HR [BPM]')
    fig.write_image('hr_fig.jpg', format='jpeg', scale=3)
    return 'speed_fig.jpg', 'hr_fig.jpg'
